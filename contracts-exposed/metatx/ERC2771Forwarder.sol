...
contract $ERC2771Forwarder is ERC2771Forwarder {
    ...
    function $_validate(ForwardRequestData calldata request) external view returns (bool alive, bool signerMatch, address signer) {
        (alive, signerMatch, signer) = super._validate(request);
    }

    function $_recoverForwardRequestSigner(ForwardRequestData calldata request) external view returns (address ret0) {
        (ret0) = super._recoverForwardRequestSigner(request);
    }
