...
contract $Address {
    ...
    function $verifyCallResult(bool success,bytes calldata returndata) external view returns (bytes memory ret0) {
        (ret0) = Address.verifyCallResult(success,returndata);
    }

    function $defaultRevert() external pure {
        Address.defaultRevert();
    }
