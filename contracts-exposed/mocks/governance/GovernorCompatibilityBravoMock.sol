...
import "../../../contracts/mocks/governance/GovernorCompatibilityBravoMock.sol";
import "../../../contracts/governance/Governor.sol";
import "../../../contracts/governance/compatibility/GovernorCompatibilityBravo.sol";
...
import "../../../contracts/governance/extensions/IGovernorTimelock.sol";
...
contract $GovernorCompatibilityBravoMock is GovernorCompatibilityBravoMock {
    bytes32 public constant __hh_exposed_bytecode_marker = "hardhat-exposed";

    event return$_cancel(uint256 proposalId);
    ...
    constructor(string memory name_, uint48 initialVotingDelay, uint32 initialVotingPeriod, uint256 initialProposalThreshold, ICompoundTimelock timelockAddress, IVotes tokenAddress) Governor(name_) GovernorSettings(initialVotingDelay, initialVotingPeriod, initialProposalThreshold) GovernorTimelockCompound(timelockAddress) GovernorVotes(tokenAddress) payable {
    }

    function $_execute(uint256 proposalId,address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 descriptionHash) external {
        super._execute(proposalId,targets,values,calldatas,descriptionHash);
    }

    function $_cancel(address[] calldata targets,uint256[] calldata values,bytes[] calldata calldatas,bytes32 salt) external returns (uint256 proposalId) {
        (proposalId) = super._cancel(targets,values,calldatas,salt);
        emit return$_cancel(proposalId);
    }
    ...
    function $_beforeExecute(uint256 arg0,address[] calldata targets,uint256[] calldata arg2,bytes[] calldata calldatas,bytes32 arg4) external {
        super._beforeExecute(arg0,targets,arg2,calldatas,arg4);
    }

    function $_afterExecute(uint256 arg0,address[] calldata arg1,uint256[] calldata arg2,bytes[] calldata arg3,bytes32 arg4) external {
        super._afterExecute(arg0,arg1,arg2,arg3,arg4);
    }
